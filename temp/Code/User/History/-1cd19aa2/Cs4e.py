...
def get_rate_info(bank_name: str | None, cibil: int, concessions: list[str] | None = None) -> str:
    """Look up home loan interest rates. Returns base and final ROI for one bank or all banks."""
    banks = build_rate_card()
    if bank_name:
        b = next((b for b in banks if b.name.lower() == bank_name.lower()), None)
        if not b:
            return json.dumps({"error": f"Bank '{bank_name}' not found", "available": [b.name for b in banks]})
        return json.dumps({
            "bank": b.name,
            "base_roi": b.get_base_roi(cibil),
            "final_roi": b.get_final_roi(cibil, concessions),
            "concessions_applied": concessions or [],
        })
    return json.dumps(roi_all(banks, cibil, concessions))
...
if __name__ == "__main__":
    print("SINGLE BANK:")
    print(get_rate_info("HDFC", 801, ["womenBorrower"]))

    print("\nALL BANKS:")
    print(get_rate_info(None, 801))
