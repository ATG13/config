...
PluginComponent {
    ...
    function displayText() {
        ...
        const effectiveLabel = value < 0 ? "overdue" : (label && label.trim().length > 0 ? label.trim() : "")
        let display
        if (unit === "hours") {
            display = formatHoursMinutes(diffMsValue)
        } else {
            display = absValue % 1 === 0 ? absValue.toFixed(0) : absValue.toFixed(1)
        }
        const pluralUnit = absValue === 1 ? unit.slice(0, -1) : unit
        const base = display + " " + pluralUnit
        return effectiveLabel ? base + " " + effectiveLabel : base
    }
    ...
    function displayTextShort() {
        ...
        if (unit === "hours") {
            ...
            return formatHoursMinutes(diffMsValue) + shortUnit() + suffix
        }
        const absValue = Math.abs(value)
    ...
    Timer {
        interval: {
            switch (root.unit) {
            case "hours": return 1000 * 30               // 30s for minute precision
