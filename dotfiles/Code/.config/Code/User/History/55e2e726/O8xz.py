...
def standard_amount_repaid(principal:float, roi:float, tenure_months:int) -> float:
    ...
    amt_repaid = emi*tenure_months

    return round(amt_repaid,0)

# Maxgain home loan calculation
def maxgain_amount_repaid(principal:float, roi:float, tenure_months:int, od_balance:float) -> float:
    """
    Calculate the Amount repaid in a standard home loan
    ...
    Args:
        ...
        roi_annual: Annual rate of interest in percent (>= 0).
        tenure_months: Loan tenure in months (> 0).
        od_balance: Amount of money you will keep in the OD account (>=0)

    Returns:
        amount_repaid: Total amount repaid at the end of the home loan
    """
    emi = calculate_emi(principal-od_balance,roi,tenure_months)

    amt_repaid = emi*tenure_months

    return round(amt_repaid,0)

print("Standard home loan stuff")
print(calculate_emi(5000000, 8.5, 60))
print(standard_amount_repaid(5000000, 8.5, 60))

print("\n")

print("Maxgain home loan stuff")
print(calculate_emi(5000000, 8.5, 60))
print(maxgain_amount_repaid(5000000, 8.5, 60, 10_00_000))